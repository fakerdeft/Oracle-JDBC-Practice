...
SELECT EMPNO,ENAME,SUBSTR(SAL,1,2)
FROM EMP
ORDER BY SAL DESC;
...
SELECT ENAME
    ,EXTRACT(YEAR FROM HIREDATE)||'년도 '||EXTRACT(MONTH FROM HIREDATE)||'월' AS 입사일
FROM EMP;

--13. EMP테이블에서 9월에 입사한 직원의 정보 조회
SELECT *
FROM EMP
WHERE EXTRACT(MONTH FROM HIREDATE) LIKE '9';

--14. EMP테이블에서 81년도에 입사한 직원 조회
SELECT *
FROM EMP
WHERE EXTRACT(YEAR FROM HIREDATE) LIKE'1981';
...
SELECT EMPNO 사번,ENAME 사원명,HIREDATE 입사일, HIREDATE+40*365 AS "입사일로부터 40년"
FROM EMP;

--18. EMP테이블에서 입사일로부터 38년 이상 근무한 직원의 정보 조회
SELECT *
FROM EMP
WHERE FLOOR(SYSDATE-HIREDATE)>=38;
